...
using DiscordTokenType = Discord.TokenType;
...
namespace KrzaqBot
{
    internal class Program
    {
        ...
        private LavalinkManager LavaLink { get; }

        public Program()
        {
            Configuration = new ConfigurationBuilder().AddUserSecrets<UserSecrets>().Build();
            Client = new DiscordSocketClient(new DiscordSocketConfig() { GatewayIntents = GatewayIntents.All });
            LavaLink = new LavalinkManager(Client, new LavalinkManagerConfig()
            {
                RESTHost = "localhost",
                RESTPort = 2333,
                WebSocketHost = "localhost",
                WebSocketPort = 2333,
                Authorization = Configuration.GetLavalinkPassword(),
                TotalShards = 1
            });
        }
        ...
        public async Task MainAsync()
        {
            ...
            //_client.GuildMemberUpdated += UserUpdatedHandler;
            Client.Ready += ClientReady;

            await Client.LoginAsync(DiscordTokenType.Bot, Configuration.GetBotToken());
