...
namespace BeziBot.Modules
{
    public class SlashCommandModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        */
        ...
        [SlashCommand("decode-emoji", "Poznaj unicode dla podanych emoji")]
        [RequireUserPermission(GuildPermission.Administrator)]
        public async Task DecodeEmoji(string text)
        {
            //string emotes = (string)command.Data.Options.First().Value;
            var codes = text.ToCharArray().TakeWhile(x => x >= 128).ToArray();
            string decoded = string.Join(string.Empty, codes.Select(c => $"\\u{(ushort)c:X4}"));
            await RespondAsync($"Emoji\n{text}\ncodes are\n\\{decoded}");
        }
        ...
        //public SlashCommandModule(InteractionHandlingService handler)
        //{
        //    _handler = handler;
        //}

        //[SlashCommand("init-music", "Let bot join to your channel")]
        //public async Task InitMusic([ChannelTypes(ChannelType.Voice)] IVoiceChannel channel = null)
        //{
        ...
        //public async Task GreetUserAsync() => await RespondAsync(text: $":ping_pong: It took me {Context.Client.Latency}ms to respond to you!", ephemeral: true);
        ...
        //    public async Task ChoiceExample(MusicProviders input) => await RespondAsync(input.ToString());
        //}
